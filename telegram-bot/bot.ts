...
import { SessionData, Step, Pack, Model, ModelStatus, Image } from './types';
import { ApiService } from './api-service';

// Загрузка переменных окружения
dotenv.config();

// Базовый тип для контекста с сессией
...
const packsMenu = new Menu<MyContext>('packs-menu');
...
async function updatePacksMenu() {
  ...
  try {
    const packs = await ApiService.getPacks();
...
const typesMenu = new Menu<MyContext>('types-menu')
...
bot.command('start', async (ctx) => {
  ...
  ctx.session.images = [];
  await ctx.reply(
    'Добро пожаловать в бота для создания аватаров! Чтобы начать, отправьте не менее 4 фотографий вашего лица в хорошем качестве.'
  );
});
...
bot.command('status', async (ctx) => {
  ...
  try {
    const status = await ApiService.getModelStatus(ctx.session.modelId);
...
bot.command('results', async (ctx) => {
  ...
  try {
    const images = await ApiService.getGeneratedImages(ctx.session.modelId);
    ...
    if (images.length > 5) {
      await ctx.reply(
        `Вы можете просмотреть все ${images.length} аватаров, перейдя по ссылке: ${API_BASE_URL}/models/${ctx.session.modelId}`
      );
    }
  } catch (error) {
...
bot.on('message:photo', async (ctx) => {
  if (ctx.session.step === 'idle' || ctx.session.step === 'upload') {
    ...
    try {
      ...
      const tempFilePath = path.join(tempDir, `${photo.file_id}.jpg`);
      fs.writeFileSync(tempFilePath, buffer);
      
      try {
        // Загружаем изображение через сервис API
        const imageUrl = await ApiService.uploadImage(buffer, `${photo.file_id}.jpg`);
...
bot.on('message:text', async (ctx) => {
  ...
  if (ctx.session.step === 'enter_name') {
    ...
    try {
      ...
      const modelId = await ApiService.createModel(
        ctx.session.images,
        ctx.session.type || 'professional',
        ctx.session.pack || '',
        ctx.session.name
      ...
      await ctx.reply(
        'Модель успешно отправлена на обучение! Этот процесс может занять некоторое время (от 15 минут до нескольких часов). ' +
...
async function getGeneratedImages(modelId: string): Promise<string[]> {
  return await ApiService.getGeneratedImages(modelId);
}
...
async function notifyUserAboutReadyAvatars(userId: string, modelId: string): Promise<void> {
  try {
    ...
    if (images.length > 0) {
      ...
      if (images.length > 5) {
        await bot.api.sendMessage(
          telegramId,
          `Вы можете просмотреть все ${images.length} аватаров, перейдя по ссылке: ${API_BASE_URL}/models/${modelId}`
        );
      }
    }
  } catch (error) {
...
export { bot, notifyUserAboutReadyAvatars }; 