...
class ApiService {
    ...
    static async uploadImage(buffer, filename) {
        try {
            ...
            const response = await axios_1.default.post(`${API_BASE_URL}/astria_mode/train-model/image-upload`, formData, {
                headers: {
                    ...formData.getHeaders(),
                },
            });
    ...
    static async getPacks() {
        try {
            const response = await axios_1.default.get(`${API_BASE_URL}/astria_mode/packs`);
     ...
     */
    static async createModel(images, type, pack, name) {
        try {
            const response = await axios_1.default.post(`${API_BASE_URL}/astria_mode/train-model`, {
                urls: images,
                type,
                pack,
                name,
     ...
     */
    static async getGeneratedImages(modelId) {
        try {
            const response = await axios_1.default.get(`${API_BASE_URL}/api/models/${modelId}/images`);
            if (response.data) {
                return response.data.map((image) => image.uri);
            }
            return [];
        }
        catch (error) {
     ...
     */
    static async getModelStatus(modelId) {
        try {
            const response = await axios_1.default.get(`${API_BASE_URL}/api/models/${modelId}`);
